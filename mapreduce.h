...
//Parameters for the function in pthread_create
struct map_param {
    struct map_reduce *mr; //map reduce structure
    int infd;              //Input file descriptor 
    int mapID;             //mapper ID 
    int nmaps;             //Number of mapper threads
};

//Parameters for reduce function
struct reduce_param {
    struct map_reduce *mr; //map reduce structure
...
struct queue_t {
    ...
    int size; //Remaining size of the buffer
    int count;//number of elements in the buffer
    pthread_mutex_t htLock; //Lock for each mapper and producer
};
 ...
 */
struct map_reduce {
    int (*map_fn)(struct map_reduce*, int, int, int);
    int (*reduce_fn)(struct map_reduce*, int, int);
    int threads;
    int buffer_size;

    int *join; //Keep track of all mapper threads, 0 for running, nonzero for exited 

    pthread_t *map_tid;   //Thread id of the mappers 
    pthread_t reduce_tid; //Thread id of the reducer 
    int *mfd;        //An array of mapper file descriptors
    int rfd;         //Reducer file descriptor  

    struct queue_t *buffer;//buffer data structure 
 
    //Locks to block produce when buffer full
